...
bool Item_func::eq(const Item *item, Eq_config config) const
{
...
bool Item_func_get_user_var::eq(const Item *item, Eq_config config) const
{
  /* Assume we don't have rtti */
  if (this == item)
...
bool Item_func_get_system_var::eq(const Item *item, Eq_config config) const
{
  /* Assume we don't have rtti */
  if (this == item)
    return 1;					// Same item is same.
...
bool Item_func_match::eq(const Item *item, Eq_config config) const
{
  if (item->type() != FUNC_ITEM ||
